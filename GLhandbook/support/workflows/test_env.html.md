...
For a GCP Testing environment, you should have access to the `support-internal` project as a baseline entitlement.
...
Specifically for *Group* SAML/SCIM (GitLab.com) testing, shared account credentials are in 1password `Okta Sandbox (SAML Testing)`.
...
+ Navigate to https://www.virtualbox.org/wiki/Downloads and download the latest version of the software
   + Download “VirtualBox (for Mac OS X)” using the provided link
   + Install VirtualBox
