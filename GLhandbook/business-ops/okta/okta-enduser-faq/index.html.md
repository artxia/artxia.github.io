...
In the case of an Okta emergency, contact GitLab Okta administrators (IT Ops) for assistance. Alternatively, there is a #okta channel on Slack.

## Is there any automated reporting? (e.g. Can we access logs emailed to the team and use that to kick off an access review?)
No, this does not exist within Okta.  But this should be something security automation team can help with. There are some automated security notifications already enabled for things like MFA or password changes, as well as conections from new and unrecognised devices. GitLab is able to perform reporting on these based on built-in reporting.
