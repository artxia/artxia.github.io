...
An issue created by a triage package is automatically assigned to team members. Those team members are listed in the triage package policy yml files. To change who an
issue gets assigned to open a MR on the appropriate policy in [the tiriage-ops repository](https://gitlab.com/gitlab-org/quality/triage-ops/tree/master/policies/).

### Newly created unlabelled issues requiring first-triage

This report contains the 45 most recent unlabelled issues requiring initial triage. The goal is to ensure we achieve [partial triage](/handbook/engineering/issue-triage#partial-triage) before the issue is picked up by a Product Manager and Engineering Manager in that area.

* Triage owner: Quality Department Engineers.
* Triage action:
  1. Check for [duplicate issues](/handbook/engineering/issue-triage/#duplicates) in this project and others (e.g. a new issue in EE may already exist in CE).
    - If identified as a duplicate bug, the new issue can be closed with a note similar to:

        ```
        @author Thanks for raising an issue. It has already been reported in the following issue:
        - #issues
    ...
    - If identified as a duplicate feature proposal, the new issue can be closed with a note similar to:
        ...
        ```
        @author Thanks for your request. It has already been proposed in the following issue:
        - #issues
  ...
  1. If the issue is a request for help:.
    * Use this template to provide resources and close the issue:
         ...
         ```
         Based on the information given, this issue is out of the scope of the Issue tracker (which is for Bug Reports and Feature Proposals). Unfortunately, I won't be able to help get it resolved. However, there are plenty of [resources](/get-help/) that you can use to find help and support from the Community, including:
          * [Reference Documents and Videos](/get-help/#references)
          * [Community Forum](https://forum.gitlab.com/)
          * [Technical Support for Paid Tiers](/support/)
...
The goal is to achieve [complete-triage](/handbook/engineering/issue-triage#complete-triage) by the Product Manager, Engineering Manager, UX team member in that area.
...
Issues which have a priority and missed the [SLO target](https://docs.gitlab.com/ee/development/contributing/issue_workflow.html#priority-labels) will be labeled with `~missed-SLO`. The calculation for elapsed time starts from the date of the priority label being applied. This enables reporting on SLO target adherence.
