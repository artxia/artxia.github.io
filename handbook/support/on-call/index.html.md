...
When you are on call you are expected to be available and ready to respond to PagerDuty pings as soon as possible, but certainly within the emergency response time set by our [Service Level Agreements](/support/#priority-support).
...
If the nature of the problem mentioned in the emergency ticket doesn't meet its
[definition of support impact](/support/#definitions-of-support-impact),
...
and, if we determine that their ticket doesn't qualify as an emergency after the call
is over, the customer's technical account manager or a support manager can discuss it
...
- First you should check with your team, see who has availability, and get confirmation from somebody that they're okay with covering for you. Generally a more advanced notice is better.
...
1. Typically, you can find the emergency ticket in `Self-managed Premium and Ultimate` queue, but in some cases the person raising the emergency may not be associated with an org. You can always use Zendesk search with the term `priority: urgent` to find the ticket.
...
1. Compare the customer's problem statement with our [definitions of support impact](/support/#definitions-of-support-impact). If you don't think that this is an actual emergency, then ask for a second opinion and kindly let the customer know if that's the decision we make.
1. If the situation is actually an emergency, then please reply to the customer and offer a Zoom call.
...
- provide ownership and assignment for tickets that are escalated in `#support-managers` (as well as redirect folks who want to escalate from the normal self-managed and .com channels)
...
To see who the current manager on-call is you can:
 ...
 - Ask in `#support-managers` (where you may or may not be referred to the above steps!)
